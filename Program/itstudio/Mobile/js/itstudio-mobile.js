/*touch.js 用途：处理200ms延迟问题*/
(function(factory){var root=(typeof self=="object"&&self.self===self&&self)||(typeof global=="object"&&global.global===global&&global);if(typeof define==="function"&&define.amd){define(["exports"],function(exports){root.Touch=factory(root,exports)})}else{if(typeof exports!=="undefined"){factory(root,exports)}else{root.Touch=factory(root,{})}}})(function(root,Touch){var root=window;var doc=document;var delegate=(function(){var T=function(){};return function(s){T.prototype=s;return new T()}})();var lastTouchTime=Date.now();function markFastClick(node,useTarget){if(useTarget){node.fastClick="usetarget"}else{node.fastClick=true}}function on(node,type,listener){node.addEventListener(type,listener);return{remove:function(){node.removeEventListener(type,listener)}}}function emit(node,type,evt){var ne=document.createEvent("HTMLEvents");ne.initEvent(type,!!evt.bubbles,!!evt.canCancel);for(var p in evt){if(!(p in ne)){ne[p]=evt[p]}}return node.dispatchEvent(ne)}function eventHandler(type){return function(node,listener){return on(node,type,function(e){if(!node.contains(e.relatedTarget,node)){listener.apply(node,arguments)}})}}function dualEvent(type){return function(node,listener){return on(node,type,listener)}}function copyEventProps(evt){return evt}function elementFromPoint(evt){var touch=evt.changedTouches[0];return doc.elementFromPoint(touch.clientX,touch.clientY)}function marked(node){while(node){if(node.fastClick){return node}node=node.parentNode}}function doFastClick(evt,moveType,endType){var markNode=marked(evt.target);var clickTracker=!evt.target.disabled&&markNode&&markNode.fastClick;if(clickTracker){var useTarget=markNode&&markNode.fastClick&&markNode.fastClick==="useTarget";var clickTarget=useTarget?markNode:evt.target;var clickX=evt.changedTouches[0].clientX;var clickY=evt.changedTouches[0].clientY;function updateClickTracker(evt){if(useTarget){clickTracker=markNode.contains(elementFromPoint(evt))?markNode:null}else{clickTracker=clickTarget===evt.target&&(Date.now()-lastTouchTime<300)&&Math.abs(evt.changedTouches[0].clientX-clickX)<4&&Math.abs(evt.changedTouches[0].clientY-clickY)<4}}doc.addEventListener(moveType,function(evt){updateClickTracker(evt);if(useTarget){evt.preventDefault()}},true);doc.addEventListener(endType,function(evt){updateClickTracker(evt);if(clickTracker){clickTime=(new Date()).getTime();var target=(useTarget?clickTarget:evt.target);if(target.tagName==="LABEL"){target=dom.byId(target.getAttribute("for"))||target}var src=(evt.changedTouches)?evt.changedTouches[0]:evt;var clickEvt=document.createEvent("MouseEvents");clickEvt._fastclick=true;clickEvt.initMouseEvent("click",true,true,evt.view,evt.detail,src.screenX,src.screenY,src.clientX,src.clientY,evt.ctrlKey,evt.altKey,evt.shiftKey,evt.metaKey,0,null);setTimeout(function(){emit(target,"click",clickEvt);clickTime=(new Date()).getTime()},0)}},true);stopNativeEvents("click");stopNativeEvents("mousedown");stopNativeEvents("mouseup");function stopNativeEvents(type){doc.addEventListener(type,function(evt){if(!evt._fastclick&&(Date.now()-clickTime)<=1000){evt.stopPropagation();evt.stopImmediatePropagation&&evt.stopImmediatePropagation();if(type=="click"&&(evt.target.tagName!="INPUT"||evt.target.type=="radio"||evt.target.type=="checkbox")&&evt.target.tagName!="TEXTAREA"&&evt.target.tagName!="AUDIO"&&evt.target.tagName!="VIDEO"){evt.preventDefault()}}},true)}}}doc.addEventListener("DOMContentLoaded",function(){var hoverNode=document.body;doc.addEventListener("touchstart",function(evt){lastTouchTime=Date.now();var newNode=evt.target;if(hoverNode){emit(hoverNode,"mytouchout",{relatedTarget:newNode,bubbles:true})}emit(newNode,"mytouchover",{relatedTarget:hoverNode,bubbles:true});hoverNode=newNode;doFastClick(evt,"touchmove","touchend")},true);doc.addEventListener("touchmove",function(evt){lastTouchTime=Date.now();var newNode=elementFromPoint(evt);if(newNode){if(newNode!==hoverNode){emit(hoverNode,"mytouchout",{relatedTarget:newNode,bubbles:true});emit(newNode,"mytouchover",{relatedTarget:hoverNode,bubbles:true});hoverNode=newNode}if(!emit(newNode,"mytouchmove",copyEventProps(evt))){evt.preventDefault()}}});doc.addEventListener("touchend",function(evt){lastTouchTime=Date.now();var newNode=elementFromPoint(evt)||doc.body;if(newNode){emit(newNode,"mytouchend",copyEventProps(evt))}})});return root.Touch=Touch={press:dualEvent("touchstart"),move:dualEvent("mytouchmove"),release:dualEvent("mytouchend"),cancel:dualEvent("touchcancel"),over:dualEvent("mytouchover"),out:dualEvent("mytouchout"),enter:eventHandler("mytouchover"),leave:eventHandler("mytouchout"),markFastClick:markFastClick}});
...
/*CarouselMobile.js 用于作品展示页*/
(function($){var Carousel=function(poster){var self=this;this.poster=poster;this.posterItemMain=poster.find("ul.poster-list");this.nextBtn=poster.find("div.poster-next-btn");this.prevBtn=poster.find("div.poster-prev-btn");this.posterItems=poster.find("li.poster-item");this.posterFirstItem=this.posterItems.first();this.posterLastItem=this.posterItems.last();this.rotateFlag=true;this.setting={"width":821,"height":390,"posterWidth":493,"posterHeight":390,"speed":1000,"autoPlay":true,"delay":5000};this.setSettingValue();this.setPosterPos();this.nextBtn.click(function(){if(self.rotateFlag){self.rotateFlag=false;self.carouseRotate("left")}});this.prevBtn.click(function(){if(self.rotateFlag){self.rotateFlag=false;self.carouseRotate("right")}});if(this.setting.autoPlay){this.autoPlay();this.poster.hover(function(){window.clearInterval(self.timer)},function(){self.autoPlay()})}};Carousel.prototype={autoPlay:function(){var self=this;this.timer=window.setInterval(function(){self.nextBtn.click()},this.setting.delay)},carouseRotate:function(dir){var _this_=this;var zIndexArr=[];if(dir==="left"){this.posterItems.each(function(){var self=$(this),prev=self.prev().get(0)?self.prev():_this_.posterLastItem,width=prev.width(),height=prev.height(),zIndex=prev.css("zIndex"),opacity=prev.css("opacity"),left=prev.css("left"),top=prev.css("top");zIndexArr.push(zIndex);self.animate({width:width,height:height,opacity:opacity,left:left,top:top},_this_.setting.speed,function(){_this_.rotateFlag=true})});this.posterItems.each(function(i){$(this).css("zIndex",zIndexArr[i])})}else{if(dir==="right"){this.posterItems.each(function(){var self=$(this),next=self.next().get(0)?self.next():_this_.posterFirstItem,width=next.width(),height=next.height(),zIndex=next.css("zIndex"),opacity=next.css("opacity"),left=next.css("left"),top=next.css("top");zIndexArr.push(zIndex);self.animate({width:width,height:height,opacity:opacity,left:left,top:top},_this_.setting.speed,function(){_this_.rotateFlag=true})});this.posterItems.each(function(i){$(this).css("zIndex",zIndexArr[i])})}}},setPosterPos:function(){var sliceItems=this.posterItems.slice(1),sliceSize=sliceItems.size()/2,rightSlice=sliceItems.slice(0,sliceSize),level=Math.floor(this.posterItems.size()/2),leftSlice=sliceItems.slice(sliceSize);rightSlice.each(function(i){level--;$(this).css({zIndex:level,width:40+"%",height:80+"%",opacity:1,left:0,top:10+"%"})});leftSlice.each(function(i){$(this).css({zIndex:i,width:40+"%",height:80+"%",opacity:1,left:60+"%",top:10+"%"})})},setSettingValue:function(){this.poster.css({width:this.setting.width,height:this.setting.height});this.posterItemMain.css({width:100+"%",height:100+"%"});this.nextBtn.css({width:20+"%",height:this.setting.height,zIndex:Math.ceil(this.posterItems.size()/2)});this.prevBtn.css({width:20+"%",height:this.setting.height,zIndex:Math.ceil(this.posterItems.size()/2)});this.posterFirstItem.css({width:this.setting.posterWidth,height:this.setting.posterHeight,left:20+"%",top:0,zIndex:Math.floor(this.posterItems.size()/2)});this.posterFirstItem.children("a").find(".hide").css({"display":"block"})}};Carousel.init=function(posters){var _this_=this;posters.each(function(){new _this_($(this))})};window["Carousel"]=Carousel})(jQuery);


/*jquery.rotate.min.js 用于导航栏的旋转 10-32行*/
...
/*jquery.easing.min.js 用于导航栏让动画流畅*/
...
/*加载页 44-97*/
$(function(){
	var realHeight=$(".container-mobile").height();
	var j=0;
	...
	var loader = new resLoader({
		...
		onComplete : function(total){
			...
			function showIndex(){
				$(".container-mobile").css({"visibility":"visible"});
				$(".container-navigation").css({"visibility":"visible"});
...
$(function(){
	...
	$("html").css({
		"fontSize":100*(cWidth/750)+"px"
	});
	if(cWidth/$(window).height()>750/1108)
		$(".container-index").css({
...
$(document).ready(function(){ 
                ...
                success: function(data) {
                    ...
                    for (j = 0; j < data.length; j++) {
                            ...
                            worksHref[j]=data[j].WorksUrl;
                            var posterItemA= $("<div>").appendTo(posterItem);
                            var posterItemAImg = $("<img>").attr("src","../BackStage/Backstage/"+ data[j].WorksImage).appendTo(posterItemA);
                    ...
                    $(".container-works").css({
                        "height":sum*$(".poster-item").height()+sum*parseFloat($(".poster-item").css("marginBottom"))+262.7*cWidth/750
                    });
                }
                });
                $(".poster-list").delegate(".poster-item>div","click",function(){
                	var worksIndex=$(".poster-item>div").index(this);
                	window.open(worksHref[worksIndex]);
                })
                /*$(".poster-item>div").click(function(){
                	var worksIndex=$(".poster-item>div").index(this);
                	window.open(worksHref[worksIndex]);
                })*/
           });
	...
	$(document).ready(function(){ 
		            ...
		            success: function (data) {
		                ...
		                for (j = 0; j < data.length; j++) {
		                    ...
		                    if (year.slice(0, 4) == "2014") {
		                        ...
		                        if (data[j].EventImage != null && data[j].EventImage != "") {
										...
										$(".clock ").on("click",function(){
								 		...
								 		$(".accomplishment-right p").find("span").on("click",function(){
											var this_=$(this);
								 			...
								 			{
									 			$(".container-event .min-top").after($("<div class='open1'></div>"),"<div class='mask1'></div>")
									 			 ...
									 			 var bottomP=$("<p>").addClass("bottomText").text($(this).text()).appendTo($(".open1"));
													$(".mask1").css(
													{
														'height':$(".container-event").css("height"),
														'width':$(".container-event").css("width")
													});
													...
													$(".open1").css({
														"top":($(".container-event").outerHeight()-$(".open1").height())/2
													});
		...
		function show(init,id){//事件进入
			...
			$(".activity-mid-line").animate({"top":(init+1)*0.7+"rem"},1,function(){//中间线的展示
				$(this).animate({"height":(10-(i)*0.7)+"rem"},300,function(){	
					...
					show_.find(".accomplishment-row").each(function (index) {
					    $(this).css({ "top": (10-(i * 0.7)) / (j + 1) * (index + 1)+"rem" })
					})
							})
						})
		}
...
$(function(){
	...
	//每一年成员的json数据
	var firstGetin=1;
    var judgeShow=new Array();//判断是展示信息还是展示图片
    var oriboxWidth=450*cWidth/750;
    var oriboxHeight=55*cWidth/750;
    var hwRatio=$(window).height()/($(window).width());
    var chaboxHeight=oriboxWidth*hwRatio/2;
    var oriboxTop=parseFloat($(".year-2011").css("top"));
    var oriboxMargin=parseFloat($(".year-2012").css("top"))-oriboxTop;
    var oriFontsize=$(".the-year").find("p").css("fontSize");
    var pageHeight=new Array(5);
		pageHeight[0]=$(".container-index").outerHeight();
		pageHeight[1]=$(".container-introduction").outerHeight();
		pageHeight[2]=$(".container-works").outerHeight();
		pageHeight[3]=$(".container-event").outerHeight();
		pageHeight[4]=$(".mobile-members").outerHeight();
	$(".the-year p").css({
		"opacity":1
	});
	$(".mobile-members").css({
		"height":$(window).height()-parseFloat($(".mobile-members").css("paddingTop"))
	});
	if(cWidth/$(window).height()>750/1108)
		$(".mobile-members").css({
			"backgroundSize":"100% auto"
	...
	$(".about-member").css({
		"left":(cWidth-$(".about-member").width())/2
	});
	$(".time-line").css({
		"top":$(window).height()/2-cWidth/50
	});
	$(".year-x").css({
		"height":chaboxHeight,
		"left":(cWidth-oriboxWidth/2)/2,
		"top":($(window).height()-chaboxHeight)/2
	});
	$(".show-year").css({
		"top":(chaboxHeight-$(".show-year").height())/2,
		"opacity":1
	});
	//对一些元素初始化定位
	...
	function timelineLocation(){
		...
		//留言板页到顶部距离
		for(var i=0;i<4;i++)
			topFirst+=pageHeight[i];
		topLast=topFirst+$(".mobile-members").outerHeight();
		//if($(window).scrollTop()<topFirst-$(".timeline-year>div").height()*(5-nowYear))
		if($(window).scrollTop()<topFirst-$(window).height()/2+cWidth/50+242*cWidth/750+$(".timeline-year>div").height()*nowYear)
		{
			$("#timeLine").css({
				"position":"absolute",
				"top":242*cWidth/750+$(".timeline-year>div").height()*nowYear
			})
		}
		//小于该范围将时间轴固定
		else if(topLast-$(window).scrollTop()<$(window).height()/2-cWidth/50+parseFloat($("#timeLine").css("marginTop"))+$("#timeLine").height()+40*cWidth/750)
		{
			$("#timeLine").css({
				"position":"absolute",
				"top":$(".mobile-members").outerHeight()-$("#timeLine").height()+$(".timeline-year>div").height()*nowYear-40*cWidth/750
			});
		}
		//大于该范围将时间轴固定
		else
	...
	$(".the-year").click(function(){
		...
		$(this).find("p").css({"color":"#e94e1b"});
		$(this).find("p").animate({
			marginTop:(chaboxHeight-oriboxHeight)/2,
			fontSize:40*cWidth/750+"px"
		},200);
		otherYear.animate({
			opacity:0.5
		},200);
		function fillscreen(){
			...
			$(".show-year").animate({
				"fontSize":80*cWidth/750,
				"top":($(window).height()-$(".show-year").height())/2,
				"opacity":0
			},300);
		}
		function initial(){
			/*初始化页面*/
			moveTotop();
			$(".real-mobile-members").fadeIn(400);
			$(".timeline-theyear img").css({
				"display":"none"
			});
			$(".timeline-theyear:eq("+nowYear+") img").css({
				"display":"block"
			});
			$(".timeline-theyear p").css({
				"fontSize":"0.22rem",
				"color":"#999"
			})
			$(".timeline-theyear p:eq("+nowYear+")").css({
				"fontSize":"0.24rem",
				"color":"#e94e1b"
			});
			timelineLocation();
			$(".time-line").css({
				"marginTop":-$(".timeline-year>div").height()*nowYear
			});
			...
			$(".the-year:eq("+nowYear+")").css({
				...
				"height":oriboxHeight,
				"left":150*cWidth/750,
				"top":oriboxTop+nowYear*oriboxMargin,
			})
	...
	$(".back-rectangle-2").click(function(){
		$(".mobile-members").css({
			"height":$(window).height()-parseFloat($(".mobile-members").css("paddingTop"))
		});
		$(".year-x").fadeOut(200,comeback);
		function comeback(){
			$(".real-mobile-members").css({"display":"none"});
			$(".year-x").css({
				"width":225*cWidth/750,
				"height":chaboxHeight,
				"left":(cWidth-oriboxWidth/2)/2,
				"top":($(window).height()-chaboxHeight)/2
			});
	...
	function refresh()
	{
		judgeShow.length=everyyearData[nowYear].length;
		for(var i=0;i<judgeShow.length;i++)
			judgeShow[i]=0;
		$(".every-member").children("div").css({
			marginLeft:0
		});
		$(".every-member").find(".member-infor").css({
			marginLeft:0
		});
	}
	//在改变当前年份时，将所有信息隐藏，并且将当前年份的judgeshow都置为0
	$(".about-member-everyyear").delegate(".every-member","touchstart",function(){
		var othermember=$(".every-member").not($(this));
		var theIndex=$(".about-member-everyyear:eq("+nowYear+")").find(".every-member").index(this);
		if(judgeShow[theIndex]==0)
		{
			judgeShow[theIndex]=1;
			...
			/*othermember.children("div").stop(false,false).animate({
				marginLeft:0
			},300);
			othermember.find(".member-infor").stop(false,false).animate({
				marginLeft:0
			},300);*/
		}
		else
		{
			judgeShow[theIndex]=0;
			$(this).children("div").stop(false,false).animate({
				marginLeft:0
			},300);
			$(this).find(".member-infor").stop(false,false).animate({
    ...
    $(".timeline-theyear").click(function(){
		usedYear=nowYear;
		nowYear=$(".timeline-theyear").index(this);
		if(usedYear==nowYear)
			return;
		moveTotop();
	...
	function moveTotop(){
		var marginHeight=0;
		for(var i=0;i<4;i++)
			marginHeight+=pageHeight[i];
		$(window).scrollTop(marginHeight);
	}
    function moveTimeline(){
    	$(".timeline-year>div img").css({
			"display":"none"
		});
		$(".timeline-year>div:eq("+nowYear+") img").css({
			"display":"block"
		});
		$(".timeline-year>div p").css({
			"fontSize":"0.22rem",
			"color":"#999"
		})
		$(".timeline-year>div:eq("+nowYear+") p").css({
			"fontSize":"0.24rem",
			"color":"#e94e1b"
		});
    ...
    function moveMember(){
		...
		refresh();//将新的一页所有图片显示，文字隐藏
		decideHeight();//决定成员页高度
    }
    function getMember(){
    	$.ajax({
			type: "GET",
		    ...
		    success: function (dat){
		    	...
		    	for(var m=0;m<6;m++)
		    	{
		    		var thisContainer=$(".about-member-everyyear:eq("+m+")");
		    		var thisyearData=everyyearData[m];
		    		for(var i=0;i<thisyearData.length;i++)
					{
					var everyMember=$("<div>").addClass("every-member").appendTo(thisContainer);
						var wholeinfor=$("<div>").addClass("clearfix").appendTo(everyMember);
    ...
    function decideHeight(){
    	$(".about-member-everyyear").css({"height":"3.5rem"});
    	$(".about-member-everyyear:eq("+nowYear+")").css({"height":"auto"});
		if(300*cWidth/750+$(".about-member").height()>$(window).height())
			$(".year-x").css({
				"height":300*cWidth/750+$(".about-member").height()
			});
		else
			$(".year-x").css({
				"height":$(window).height()
			...
			$(".mobile-members").css({
				"height":$(".year-x").height()-parseFloat($(".mobile-members").css("paddingTop"))-2
			});
    }
})
...
$(function(){
			var cWidth=document.body.clientWidth;
	        $(".join-us").css({
	        	"top":$(window).height()/2-100*cWidth/750
	        });
	        $(window).resize(function(){
	        	$(".join-us").css({
	        		"top":$(window).height()/2-100*cWidth/750
			...
			$(".roll-li ").click(function(e){
       			 var number= $(".roll-li").index(this);
       			 closeNavigation();
       			 var marginHeight=0;
       			 pageHeight[4]=$(".mobile-members").innerHeight();
			...
			$(".join-us").click(function(){
				window.location.href="ApplicationFormMobile.aspx";
			})
    	...
    	function open(){//弹出弹出层
			...
			/*var top_=(H*0.4);*/
			var top_=($(window).height()-$(".open").height())/2-50*cWidth/750;
			$(".mask").css(
			{
				'height':rH,
				'width':100+'%'
...
$(function () {
	...
	var i=0;
	var afterLength=0;
	var firstP=parseFloat($(".photo-tick").css("left"));
	appendcomment();
	...
	function appendcomment(){
		...
		else
		{
			i=1;
			overLoad=false;
			$(".comments-contain").find(".every-comment").remove();
		}
		$.ajax({
            type:"GET",
            ...
            success:function(dat){
            	if(dat=="[]"){
            		...
            		$(".more-comment").css({"display":"block"});
                	jsonObj=JSON.parse(dat);
            		for(var j=0;j<jsonObj.length;j++)
            		{
            			var everyComments=$("<div>").addClass("every-comment").appendTo($(".comments-contain"));
            				var commentsInf=$("<div>").addClass("comments-inf").addClass("clearfix").appendTo(everyComments);
            					var visitorPho=$("<div>").addClass("visitor-photo").appendTo(commentsInf);
            						var visitorImg=$("<img>").attr("src",jsonObj[j].MessagePhoto).appendTo(visitorPho);
            					var aboutV=$("<div>").addClass("about-visitor").appendTo(commentsInf);
            						var visitorName=$("<h1>").appendTo(aboutV).text("匿名留言");
            						var visTime=$("<p>").text(jsonObj[j].MessageTime.replace("T", " ")).appendTo(aboutV);
            						 //呈现留言时解码
            						 var comments=htmlDecodeJQ(jsonObj[j].MessageContent);
            				var commentsCon=$("<div>").addClass("comments-content").text(comments).appendTo(everyComments)
            				if(jsonObj[j].MessageComment!=null)
            				{
            				var adminR=$("<div>").addClass("admin-reply").addClass("clearfix").appendTo(everyComments);
	            				var adminh=$("<h1>").text("管理员回复").appendTo(adminR);
	            				var adminP=$("<p>").text(jsonObj[j].MessageComment).appendTo(adminR);
            				}
