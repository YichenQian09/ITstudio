            ...
            if (ans > 0)//能否写入IP
            {
                if (ans == 1)//该IP十秒内无操作
                {
                    ...
                    using (var db = new ITShowEntities())
                    {
                        ...
                        if (db.SaveChanges() == 1)
                            Response.Write("<script>alert('报名成功，欢迎加入爱特交流群，群号:245775349');location='ApplicationForm.aspx'</script>");
                        else
                            Response.Write("<script>alert('报名失败请重试');location='ApplicationForm.aspx'</script>");
                    }
                }
                else
                    Response.Write("<script>alert('操作过于频繁请10秒后再操作');location='ApplicationForm.aspx'</script>");
            }
            else
                Response.Write("<script>alert('操作失败请重试');location='ApplicationForm.aspx'</script>");
        }
    }
}